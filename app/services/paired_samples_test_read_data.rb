class PairedSamplesTestReadData
  def self.base_object
    object = {
      products: [
        "GI68",
        "SU74",
        "PO32",
        "CA52",
        "FL49"
      ],

      questions: {
        "Q3": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q3.TB": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q3.T2B": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q3.T3B": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q3.B2B": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q4.JR": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q4.STRONG": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q4.WEAK": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q5": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q5.TB": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q5.T2B": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q5.T3B": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q5.B2B": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q6.JR": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q6.STRONG": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q6.WEAK": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q7": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q7.TB": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q7.T2B": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q7.T3B": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q7.B2B": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R1": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R2": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R3": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R4": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R5": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R6": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R7": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R8": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R9": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R10": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R11": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R12": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R13": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q8.R14": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q9": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q9.TB": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q9.T2B": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
        "Q9.B2B": {
          "GI68": { means: 0.0 },
          "SU74": { means: 0.0 },
          "PO32": { means: 0.0 },
          "CA52": { means: 0.0 },
          "FL49": { means: 0.0 }
        },
      }
    }
  end

  def self.read_data_without_benchmark(object, data_file)
    xlsx = Roo::Spreadsheet.open(data_file)

    object.questions.each do |question|
      object.products.keys.each do |p|
        object.products[p][question.to_s] = {}
      end
    end

    xlsx.each_with_pagename do |name, sheet|
      object.questions.push name.to_s if object.questions.include?(name) == false && name.downcase.start_with?("sheet") == false

      st_flag, tt_flag, corr_flag = false, false, false
      bm_name = nil

      sheet.each_with_index do |row, index|
        p index
        next if (row.reject { |x| x.nil? }).empty?
        name = name || nil
        if row[0].to_s.downcase == "syntax"
          object.questions.each do |question|
            if row[2].to_s.include?("#{question}")
              name = x
              st_flag, tt_flag, corr_flag = false, false, false
              bm_name = nil
            end
          end
        end

        if row[0].to_s.downcase.include?(object.statistic_table)
          st_flag = true
          corr_flag = false
          next
        end

        if st_flag == true && corr_flag == false && ( row[0].to_s.downcase.start_with?("pair ") )
          # First Row is Benchmark Product
          # Second Row is Current Product
          bm_name = nil
          product_name = nil
          object.products.keys.each do |key|
            bm_name = key if row[1].include?(key)
            product_name = key if sheet.row(index + 3)[1].include?(key)
          end

          object.products[bm_name][name][:mean] ||= (row[3].to_f > 1.0 ? row[3].to_f : (row[3] == "." ? "." : row[3].to_f * 100) )
          object.products[bm_name][name][:compare_with] ||= {}
          object.products[bm_name][name][:compare_with][product_name] ||= {}
          object.products[bm_name][name][:compare_with][product_name][:mean] ||= (sheet.row(index + 3)[2].to_f > 1.0 ? sheet.row(index + 3)[2].to_f : (sheet.row(index + 3)[2] == "." ? "." : sheet.row(index + 3)[2].to_f * 100) )

        end

        if row[0].to_s.downcase.include?(object.correlation)
          st_flag = false
          corr_flag = true
        end

        if row[0].to_s.downcase.include?(object.test_table)
          st_flag = false
          corr_flag = false
          tt_flag = true
          next
        end

        if tt_flag == true && st_flag == false && row[0].to_s.downcase.start_with?("pair ")
          product_name = nil
          object.products[bm_name][name][:compare_with].keys.each do |key|
            product_name = key if row[1].to_s.include?(key)
          end
          next if product_name.nil?
          current_product_mean = object.products[bm_name][name][:mean]
          compare_with_mean = object.products[bm_name][name][:compare_with][product_name][:mean]
          object.products[bm_name][name][:compare_with][product_name].merge!({
            score: row[9].to_f,
            test_80: (row[9].to_f  < 0.2 ? (current_product_mean > compare_with_mean ? "W" : "L") : nil),
            test_90: (row[9].to_f  < 0.1 ? (current_product_mean > compare_with_mean ? "W" : "L") : nil),
            test_95: (row[9].to_f  < 0.05 ? (current_product_mean > compare_with_mean ? "W" : "L") : nil),
            test_99: (row[9].to_f  < 0.01 ? (current_product_mean > compare_with_mean ? "W" : "L") : nil)
          })
        end

      end # End Sheet

    end # End Sheets
    object
  end

  def self.read_data_with_benchmark(object, data_file)
    xlsx = Roo::Spreadsheet.open(data_file)

    object.questions.each do |question|
      object.benchmarks.keys.each do |p|
        object.benchmarks[p][question.to_s] = {}
      end
      object.products.keys.each do |p|
        object.products[p][question.to_s] = {}
      end
    end

    xlsx.each_with_pagename do |name, sheet|
      object.questions.push name.to_s if object.questions.include?(name) == false && name.downcase.start_with?("sheet") == false

      st_flag, tt_flag = false, false
      product_name = nil

      sheet.each_with_index do |row, index|
        p index
        next if (row.reject { |x| x.nil? }).empty?
        next if row[0].present? && row[0].start_with?("No statistics are computed for a split file")
        name = name || nil

        if row[0].to_s.downcase == "syntax"
          object.questions.each do |question|
            if row[2].to_s.include?("#{question}")
              name = x
              st_flag, tt_flag = false, false
              product_name = nil
            end
          end
        end

        product_name = 'PA99'

        if row[0].to_s.downcase.start_with?("product =")
          product_name = row[0].split("=").second.split(" ").join(" ")
          st_flag, tt_flag = false, false
          next
        end

        if row[0].to_s.downcase.include?(object.statistic_table)
          st_flag = true
          next
        end

        if st_flag == true && ( row[0].to_s.downcase.start_with?("pair ") || sheet.row(index)[0].to_s.downcase.start_with?("pair "))

          count = 0
          object.benchmarks.keys.each do |key|
            if (row[1].to_s.include?(name + "_") || row[1].to_s == name)&& object.benchmarks[key][name][:mean].nil?
              object.benchmarks[key][name][:mean] = (row[2].to_f > 1.0 ? row[2].to_f : (row[2] == "." ? "." : row[2].to_f * 100 ) ) if object.benchmarks[key][name][:mean].nil?
            else
              count += 1
            end
          end

          if count == object.benchmarks.keys.length && object.products[product_name][name][:mean].nil?
            object.products[product_name][name][:mean] = (row[2].to_f > 1.0 ? row[2].to_f : (row[2] == "." ? "." : row[2].to_f * 100) )
          end
        end

        if row[0].to_s.downcase.include?(object.test_table)
          st_flag = false
          tt_flag = true
          next
        end

        if tt_flag == true && st_flag == false
          object.products[product_name][name][:compare_with] ||= {}
          object.benchmarks.keys.each do |key|
            object.products[product_name][name][:compare_with][key] ||= {
              score: 0,
              test_80: nil,
              test_90: nil,
              test_95: nil,
              test_99: nil
            }
          end
        end

        if tt_flag == true && st_flag == false && row[0].to_s.downcase.start_with?("pair ")
          compare_with = nil
          object.benchmarks.keys.each do |key|
            compare_with = key if row[1].to_s.include?(key)
          end
          next if compare_with.nil?
          compare_with_mean = object.benchmarks[compare_with][name][:mean]

          current_product_mean = object.products[product_name][name][:mean] || 3.0
          compare_with_mean ||= current_product_mean
          current_product_mean ||= 3.0

          object.products[product_name][name][:compare_with][compare_with] = {
            score: row[9].to_f,
            test_80: ( row[9].to_f  < 0.2 ? (current_product_mean > compare_with_mean ? "W" : "L") : nil ),
            test_90: ( row[9].to_f  < 0.1 ? (current_product_mean > compare_with_mean ? "W" : "L") : nil ),
            test_95: ( row[9].to_f  < 0.05 ? (current_product_mean > compare_with_mean ? "W" : "L") : nil ),
            test_99: ( row[9].to_f  < 0.01 ? (current_product_mean > compare_with_mean ? "W" : "L") : nil )
          }

        end
      end # End Sheet

    end # End

    object
  end # End Method
end
